...
#include <flux/setup/endian.hpp>
#include <flux/util/debug_optimize.hpp>

/* Modified implementation of the CRC32 slicing-by-4 code by Stephan Brumme
...
namespace
{
	constexpr flux::uint32_t kCrc32Lookup_[4][256] =
	{
		/* Tables for slicing-by-4. See original Crc32.cpp. */
		{
...
namespace flux_ext
{
	flux::uint32_t crc32_fast( void const* aData, std::size_t aLength, flux::uint32_t aPreviousCRC32 ) noexcept
	{
		static constexpr std::uintptr_t kAlign_ = 4; // 4bytes, must be POT

		flux::uint32_t crc = ~aPreviousCRC32; // ~x <=> x^0xffffffff

		// Try to process 4-byte words at a time.
		if( aLength >= kAlign_ )
		{
			// Peel loop to make sure it's aligned sufficiently for 32-bit words
			auto const ibptr = reinterpret_cast<flux::uintptr_t>(aData);
			auto const abptr = (ibptr + kAlign_-1) & ~(kAlign_-1);

			auto const* ubeg = reinterpret_cast<flux::uint8_t const*>(ibptr);
			auto const* abeg = reinterpret_cast<flux::uint8_t const*>(abptr);

			for( ; ubeg != abeg; ++ubeg )
				crc = (crc >> 8) ^ kCrc32Lookup_[0][(crc & 0xff) ^ *ubeg];

			aLength -= (abptr-ibptr);
			aData = abeg;

			// Handle bulk in 4-byte words
			auto const ieptr = ibptr + aLength;
			auto const aeptr = ieptr & ~(kAlign_-1);
		
			if( aeptr > abptr )
			{
				auto const* aend = reinterpret_cast<flux::uint32_t const*>(aeptr);
				auto const* wbeg = reinterpret_cast<flux::uint32_t const*>(abeg);

				for( ; wbeg != aend; ++wbeg )
				{
#					if FLUX_ENDIAN == FLUX_ENDIAN_BIG
					auto const one = *wbeg ^ swap(crc);
					crc = kCrc32Lookup_[0][ one      & 0xffu] ^
						  kCrc32Lookup_[1][(one>> 8) & 0xffu] ^
						  kCrc32Lookup_[2][(one>>16) & 0xffu] ^
						  kCrc32Lookup_[3][(one>>24) & 0xffu];
#					elif FLUX_ENDIAN == FLUX_ENDIAN_LITTLE
					auto const one = *wbeg ^ crc;
					crc = kCrc32Lookup_[0][(one>>24) & 0xffu] ^
						  kCrc32Lookup_[1][(one>>16) & 0xffu] ^
						  kCrc32Lookup_[2][(one>> 8) & 0xffu] ^
						  kCrc32Lookup_[3][ one      & 0xffu];
#					else
...
#					endif // ~ endian
		...
		// Handle tail
		auto const* bbeg = reinterpret_cast<flux::uint8_t const*>(aData);
		auto const* uend = bbeg+aLength;

		for( ; bbeg != uend; ++bbeg )
			crc = (crc >> 8) ^ kCrc32Lookup_[0][(crc & 0xff) ^ *bbeg];
