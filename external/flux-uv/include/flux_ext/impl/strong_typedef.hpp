// **flux-MODIFIED**
// The original copyright and license statement has been moved to line 8. The
// LICENSE file mentioned therein has been moved to docs/LICENSE.type_safe in
// the flux-ext_misc_ directory. Other modifications are marked with the
// **flux-MODIFIED** marker. See https://github.com/foonathan/type_safe for
// original sources.
...
namespace type_safe
{
    ...
    public:
        ...
        /// \group value_conv
        explicit FLUX_CONSTEXPR_EX operator T&() TYPE_SAFE_LVALUE_REF noexcept
        {
            return value_;
        }
        ...
        /// \group value_conv
        explicit FLUX_CONSTEXPR_EX operator T &&() && noexcept
        {
    ...
    template <class Tag, typename T>
    FLUX_CONSTEXPR_EX T& get(strong_typedef<Tag, T>& type) noexcept
    {
    ...
    namespace strong_typedef_op
    {
        ...
        namespace detail
        {
            ...
            template <class StrongTypedef>
            FLUX_CONSTEXPR_EX underlying_type<StrongTypedef>& get_underlying(
                StrongTypedef& type)
            {
            ...
            // **flux-MODIFIED** VisualStudio 2013 compat
#           if FLUX_COMPILER != FLUX_COMPILER_MSVC || _MSC_VER > 1800 // TODO maybe VS2015.update 1...
...
#           else // Visual Studio 2013
            ...
            struct is_strong_typedef< strong_typedef<tTag,tType> >
                : std::true_type {};
...
#define TYPE_SAFE_DETAIL_MAKE_OP_COMPOUND(Op, Name)                                                \
    /** \exclude */                                                                                \
    template <class StrongTypedef>                                                                 \
    FLUX_CONSTEXPR_EX StrongTypedef& operator Op(Name<StrongTypedef>&       lhs,               \
    ...
    template <class StrongTypedef, typename Other,                                                 \
              typename = detail::enable_if_convertible<Other&&, StrongTypedef>>                    \
    FLUX_CONSTEXPR_EX StrongTypedef& operator Op(Name<StrongTypedef>& lhs, Other&& rhs)        \
    {                                                                                              \
        detail::get_underlying<StrongTypedef>(static_cast<StrongTypedef&>(lhs))                    \
            Op detail::get_underlying<StrongTypedef>(detail::forward<Other>(rhs));                 \
        return static_cast<StrongTypedef&>(lhs);                                                   \
...
#define TYPE_SAFE_DETAIL_MAKE_OP_COMPOUND_MIXED(Op, Name)                                          \
    ...
    template <class StrongTypedef, typename OtherArg, typename Other,                              \
              typename = detail::enable_if_convertible_same<Other&&, OtherArg>>                    \
    FLUX_CONSTEXPR_EX StrongTypedef& operator Op(Name<StrongTypedef, OtherArg>& lhs,           \
                                                     Other&&                        rhs)           \
    {                                                                                              \
...
#define TYPE_SAFE_DETAIL_MAKE_STRONG_TYPEDEF_OP(Name, Op)                                          \
    ...
    TYPE_SAFE_DETAIL_MAKE_OP_COMPOUND_MIXED(Op## =, mixed_##Name##_noncommutative)
        ...
        {
            /// \exclude
            FLUX_CONSTEXPR_EX StrongTypedef& operator++()
            {
            ...
            /// \exclude
            FLUX_CONSTEXPR_EX StrongTypedef operator++(int)
            {
...
// **flux-MODIFIED**:
//   - cleanup defines
#undef TYPE_SAFE_LVALUE_REF
#undef TYPE_SAFE_RVALUE_REF
#undef TYPE_SAFE_USE_REF_QUALIFIERS
