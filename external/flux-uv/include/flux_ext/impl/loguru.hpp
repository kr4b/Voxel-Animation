...
#define LOGURU_PREDICT_TRUE(x)  (__builtin_expect(!!(x), 1))
	...
	// **flux-MODIFIED** use fmtlib included in flux
...
#undef MAX
	...
	*/
...
#if LOGURU_USE_FMTLIB
	// **flux-MODIFIED** this seems to refer to an older version of fmtlib.
	// Mine doesn't define FMT_VARIADIC() any more. Either way: the following
	// should work...
	...
	void log(Verbosity verbosity, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, tArgs&&... aArgs )
	{
		vlog( verbosity, file, line, format, fmt::make_format_args(std::forward<tArgs>(aArgs)...) );
	}
	// Log without any preamble or indentation.
	void vraw_log(Verbosity verbosity, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, fmt::format_args args);
	//FMT_VARIADIC(void, raw_log, Verbosity, const char*, unsigned, LOGURU_FORMAT_STRING_TYPE)
	template< typename... tArgs >
	void raw_log(Verbosity verbosity, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, tArgs&&... aArgs )
	{
		vraw_log( verbosity, file, line, format, fmt::make_format_args(std::forward<tArgs>(aArgs)...) );
	}
	...
	public:
		...
		/* Without mandatory copy elision (hello, visual studio), as the
		 * destructor ends up being called twice. Thus, set the _file 
		 * manually to nullptr when moving.
		 */
//		LogScopeRAII(LogScopeRAII&& other) = default;
		...
		{
			std::memcpy( _name, other._name, sizeof(_name) );
			other._file = nullptr;
		}
...
#define CHECK_OP_F(expr_left, expr_right, op, ...)                                                 \
	...
	// **flux-MODIFIED** VisualStudio errors out if "c" is not named but part of __VA_ARGS__ for some reason (bug?)
	#define DLOG_IF_F(verbosity_name, c, ...)  LOG_IF_F(verbosity_name, c, __VA_ARGS__)
	#define DVLOG_IF_F(verbosity, c, ...)      VLOG_IF_F(verbosity, c, __VA_ARGS__)
...
#	if !defined(NOMINMAX) // **flux-MODIFIED** flux predefines this.
#		define NOMINMAX
#	endif // ~ NOMINMAX
	#include <windows.h>
#endif
...
#if defined(_MSC_VER)
#	define strdup _strdup // **flux-MODIFIED** avoid unnecessary deprecation spam

#	if _MSC_VER <= 1800
#		define snprintf _snprintf // **flux-MODIFIED** no snprintf on VisualStudio 2013
...
#define VTSEQ(ID) ("\e[" #ID "m")
	...
	void init(int& argc, char* argv[], const char* verbosity_flag)
	{
		...
		if (g_stderr_verbosity >= Verbosity_INFO) {
			if (g_preamble) {
				...
				// **flux-MODIFIED** HACK: don't print as wide to stderr
				auto const* ptr = std::strchr( preamble_explain, '[' );
				if( !ptr ) ptr = preamble_explain;
					
				if (g_colorlogtostderr && s_terminal_has_color) {
					fprintf(stderr, "%s%s%s\n", terminal_reset(), terminal_dim(), ptr);
				} else {
					fprintf(stderr, "%s\n", ptr);
				}
			}
	...
	static void print_preamble_header(char* out_buff, size_t out_buff_size)
	{
		size_t pos = 0; // **flux-MODIFIED** avoid signed/unsigned mismatch
	...
	static void log_message(int stack_trace_skip, Message& message, bool with_indentation, bool abort_if_fatal)
	{
		...
		if (verbosity <= g_stderr_verbosity) {
			// **flux-MODIFIED**
			// Hack: reduce the preamble a bit when logging to stderr, it's too
			// much wasted space for my taste.
			auto const* pre = std::strchr( message.preamble, '[' );
			if( !pre )
				pre = message.preamble;
			...
			if (g_colorlogtostderr && s_terminal_has_color) {
				...
				fprintf(stderr, "%s%s%s%s\n",
					pre, message.indentation, message.prefix, message.message);
			}
	...
	void vraw_log(Verbosity verbosity, const char* file, unsigned line, const char* format, fmt::format_args args)
	{
		auto formatted = fmt::vformat(format, args);
