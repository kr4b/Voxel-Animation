...
// **flux-MODIFIED** Original LICENSE.txt can be found in docs/LICENSE.variant.)
...
#if !defined(variant_HAVE_STD_VARIANT) // **flux-MODIFIED**
...
#if variant_HAVE_STD_VARIANT // **flux-MODIFIED**
#include <variant>
...
#  if !defined(variant_HAVE_CONSTEXPR_14) // **flux-MODIFIED**
...
//**flux-MODIFIED** added: in_place
const in_place_t in_place{};

#if 0
template< class T >
inline in_place_t in_place_type( detail::in_place_type_tag<T> = detail::in_place_type_tag<T>() ) //**flux-MODIFIED** this was called in_place and not in_place_type
{
    return in_place_t();
}

template< std::size_t I >
inline in_place_t in_place_type( detail::in_place_index_tag<I> = detail::in_place_index_tag<I>() )
{
    return in_place_t();
}
#endif
...
#undef nonstd_lite_HAVE_IN_PLACE_TYPES // **flux-MODIFIED** 
...
#endif // **flux-MODIFIED** not sure why const& constructors shouldn't be made available...
...
#endif // **flux-MODIFIED** Not sure what this is about...
    template< class T, std::size_t I >
...
//#endif // variant_CPP11_OR_GREATER  // **flux-MODIFIED** Not sure what this is about...
...
inline R & get( variant<T0, T1, T2, T3, T4, T5, T6> & v, nonstd_lite_in_place_type_t(R) = in_place_type<R> )
{
    return v.template get<R>();
}

template< class R, class T0, class T1, class T2, class T3, class T4, class T5, class T6  >
inline R const & get( variant<T0, T1, T2, T3, T4, T5, T6> const & v, nonstd_lite_in_place_type_t(R) = in_place_type<R> )
{
    return v.template get<R>();
}
...
get( variant<T0, T1, T2, T3, T4, T5, T6> & v, nonstd_lite_in_place_index_t(I) = in_place_index<I> )
{
    if ( I != v.index() )
    {
        throw bad_variant_access();
...
get( variant<T0, T1, T2, T3, T4, T5, T6> const & v, nonstd_lite_in_place_index_t(I) = in_place_index<I> )
{
    if ( I != v.index() )
    {
        throw bad_variant_access();
...
get_if( variant<T0, T1, T2, T3, T4, T5, T6> * pv, nonstd_lite_in_place_type_t(T) = in_place_type<T> )
{
...
get_if( variant<T0, T1, T2, T3, T4, T5, T6> const * pv, nonstd_lite_in_place_type_t(T) = in_place_type<T>)
{
    return ( pv->index() == pv->template index_of<T>() ) ? &get<T>( *pv ) : variant_nullptr;
}
...
get_if( variant<T0, T1, T2, T3, T4, T5, T6> * pv, nonstd_lite_in_place_index_t(I) = in_place_index<I> )
{
...
get_if( variant<T0, T1, T2, T3, T4, T5, T6> const * pv, nonstd_lite_in_place_index_t(I) = in_place_index<I> )
{
    return ( pv->index() == I ) ? &get<I>( *pv )  : variant_nullptr;
}
