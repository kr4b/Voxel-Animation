...
// **flux-MODIFIED** Several modifications, look for **flux-MODIFIED**
// **flux-MODIFIED** Specific fixes that are unmarked at each location:
// **flux-MODIFIED**   - rename short macro names that would collide with
// **flux-MODIFIED**     other stuff (e.g., windows.h)
...
#include <flux/setup/compiler.hpp> // **flux-MODIFIED**
#include <flux/setup/stdfeat.hpp> // **flux-MODIFIED**

namespace cxx_function {

#if __cplusplus > 201610 && !FLUX_STDQUIRK_CXX17_MISSING_STDLIB17 // **flux-MODIFIED**
    template< typename t >
...
#define FLUXTMP_UGLYY( NAME ) CXX_FUNCTION_ ## NAME

#if FLUX_COMPILER_FAMILY == FLUX_COMPILER_GCC && __GNUC__ < 5 // **flux-MODIFIED**
...
#   define FLUXTMP_deprecated(MSG) __attribute__((deprecated (MSG)))

#   define FLUXTMP_OLD_GCC_FIX(...) __VA_ARGS__
#   define FLUXTMP_OLD_GCC_SKIP(...)
#else
#   define FLUXTMP_deprecated(MSG) [[deprecated (MSG)]]

#   define FLUXTMP_OLD_GCC_FIX(...)
#   define FLUXTMP_OLD_GCC_SKIP(...) __VA_ARGS__
#endif

#if FLUX_COMPILER_FAMILY == FLUX_COMPILER_MSVC // **flux-MODIFIED**
#   define FLUXTMP_MSVC_FIX(...) __VA_ARGS__
#   define FLUXTMP_MSVC_SKIP(...)
#else
#   define FLUXTMP_MSVC_FIX(...)
#   define FLUXTMP_MSVC_SKIP(...) __VA_ARGS__
#endif

#define FLUXTMP_UNPACK(...) __VA_ARGS__
#define FLUXTMP_IGNORE(...)

#define DISPATCH_CQ( MACRO, UNSAFE, TYPE_QUALS, FN_QUALS ) \
    MACRO( TYPE_QUALS, FN_QUALS, UNSAFE ) MACRO( const TYPE_QUALS, const FN_QUALS, FLUXTMP_IGNORE )
#define DISPATCH_CV( MACRO, UNSAFE, TYPE_QUALS, FN_QUALS ) \
    DISPATCH_CQ( MACRO, UNSAFE, TYPE_QUALS, FN_QUALS ) DISPATCH_CQ( MACRO, FLUXTMP_IGNORE, volatile TYPE_QUALS, volatile FN_QUALS )

// Apply a given macro over all reference qualifications.
#define DISPATCH_CVREFQ( MACRO, TYPE_QUALS, FN_QUALS ) \
    DISPATCH_CV( MACRO, FLUXTMP_IGNORE, & TYPE_QUALS, & FN_QUALS ) DISPATCH_CV( MACRO, FLUXTMP_IGNORE, && TYPE_QUALS, && FN_QUALS )
...
#if defined(__cpp_noexcept_function_type) && __cpp_noexcept_function_type >= 201510 // **flux-MODIFIED**
#   define DISPATCH_ALL( MACRO ) DISPATCH_CVOBJQ( MACRO, FLUXTMP_UNPACK, , ) DISPATCH_CVOBJQ( MACRO, FLUXTMP_IGNORE, , noexcept )
#else
#   define DISPATCH_ALL( MACRO ) DISPATCH_CVOBJQ( MACRO, FLUXTMP_UNPACK, , )
#endif

#if defined(__cpp_lib_experimental_logical_traits) && __cpp_lib_experimental_logical_traits >= 201511 // **flux-MODIFIED**; NOTE that this is technically incorrect, since the preprocessor value is defined in the header <experimental/type_traits>, which we would have to __has_include and then include first. Yes, it's dumb.
...
T launder_cast( void * p )
    { return
#if FLUX_STDFEAT_CXX17_LAUNDER // **flux-MODIFIED**
...
struct make_erasure_table {
    static FLUXTMP_MSVC_SKIP (constexpr) erasure_table< typename member_to_free< sig >::type ... > const value FLUXTMP_MSVC_SKIP( VTABLE_INITIALIZER );
};
#ifdef __GNUC__
#   pragma GCC diagnostic pop
#endif
template< typename erasure, typename copyable, typename ... sig >
FLUXTMP_MSVC_SKIP (constexpr) erasure_table< typename member_to_free< sig >::type ... > const
make_erasure_table< erasure, copyable, sig ... >::value FLUXTMP_MSVC_FIX( VTABLE_INITIALIZER );
...
struct allocator_erasure
    ...
    static FLUXTMP_MSVC_SKIP (constexpr) std::type_info const * const common_allocator_type_info
        FLUXTMP_MSVC_SKIP ( = & typeid (allocator_erasure::common_allocator) );
...
public:
    ...
    static void move( erasure_base && self_base, void * dest, void const * dest_allocator_v ) {
        ...
        std::move( self ).move( FLUXTMP_MSVC_FIX (impl::) is_always_equal< allocator_in >{}, dest, dest_allocator_v );
    }
...
template< typename allocator_in, typename in_target_type >
FLUXTMP_MSVC_SKIP (constexpr) std::type_info const * const allocator_erasure< allocator_in, in_target_type >::common_allocator_type_info
    FLUXTMP_MSVC_FIX ( = & typeid (allocator_erasure::common_allocator) );
...
#if FLUX_STDFEAT_CXX17_INVOKE // **flux-MODIFIED**
...
struct is_callable< t, ret( arg ... ) FN_QUALS, \
    ...
    , ret >::value >::type > \
    : std::true_type {};
DISPATCH_CVOBJQ( IS_CALLABLE_CASE, FLUXTMP_IGNORE, , )
...
#if defined(__cpp_noexcept_function_type) && __cpp_noexcept_function_type >= 201510 && !FLUX_STDQUIRK_CXX17_MISSING_STDLIB17 // **flux-MODIFIED**
...
#   define NOEXCEPT_CASE( TYPE_QUALS, FN_QUALS, UNSAFE ) \
    ...
    DISPATCH_CVOBJQ( NOEXCEPT_CASE, FLUXTMP_IGNORE, , )
#   undef NOEXCEPT_CASE
...
#   define NOEXCEPT_NULLPTR_CASE( TYPE_QUALS, FN_QUALS, UNSAFE ) \
    ...
    DISPATCH_CVOBJQ( NOEXCEPT_NULLPTR_CASE, FLUXTMP_IGNORE, , noexcept )
...
#define WRAPPER_CASE( \
    ...
    UNSAFE /* FLUXTMP_UNPACK if there are no qualifiers, FLUXTMP_IGNORE otherwise. Supports deprecated const-qualified access. */ \
) \
...
struct wrapper_dispatch< derived, n, const_unsafe_case< ret( arg ... ) >, more ... >
    ...
    FLUXTMP_deprecated( "It is unsafe to call a std::function of non-const signature through a const access path." )
...
typename std::enable_if< conjunction<
    std::is_same< target, typename target::FLUXTMP_UGLYY(wrapper_type) >
...
protected: // Queries on potential targets. Shared with container_wrapper.
    ...
    template< typename source >
    struct is_compatibly_wrapped< source, typename source::FLUXTMP_UGLYY(wrapper_type), typename source::wrapper_base >
        : std::true_type {};
...
public:
    typedef wrapper FLUXTMP_UGLYY(wrapper_type);
...
FLUXTMP_MSVC_SKIP ( // MSVC cannot parse deprecated constructor templates, so remove this entirely.
    template< typename allocator >
    FLUXTMP_deprecated( "This constructor ignores its allocator argument. Specify allocation per-target or use function_container instead." )
...
typename source::FLUXTMP_UGLYY(common_allocator) && select_allocator( std::true_type, void *, source const * s )
...
protected:
    ...
    template< typename source >
    struct has_compatible_allocator< source, typename source::FLUXTMP_UGLYY(common_allocator) > : std::true_type {};
    
    typedef std::integral_constant< bool,
        allocator_traits::propagate_on_container_move_assignment::value || FLUXTMP_MSVC_FIX (impl::) is_always_equal< allocator_in >::value > noexcept_move_assign;
public:
    ...
    typedef allocator_in FLUXTMP_UGLYY(common_allocator);
    
    container_wrapper() = default;
    explicit container_wrapper( allocator_in const & alloc ) noexcept
        : allocator_in{ alloc } {}
    FLUXTMP_deprecated( "std::allocator_arg is now unnecessary in function_container classes. Just pass the allocator instead." )
    container_wrapper( std::allocator_arg_t, allocator_in const & in_alloc ) noexcept
        : FLUXTMP_UGLYY(common_allocator){ in_alloc } {}
    ...
    noexcept( is_noexcept_erasable< typename std::decay< source >::type >::value
            || ( is_compatibly_wrapped< source >::value && FLUXTMP_MSVC_FIX (impl::) is_always_equal< allocator_in >::value ) )
    ...
    operator = ( source && s ) { // Throws allocator_mismatch_error.
        return this->nontrivial_target( s, & saved_allocator() ) // Verify dynamic allocator compatibility even if is_always_equal.
            && ! FLUXTMP_MSVC_FIX (impl::) is_always_equal< allocator_in >::value // POCMA doesn't apply here.
            ? assign( typename FLUXTMP_MSVC_FIX (impl::) is_always_equal< allocator_in >::type{}, std::move( s ) ) // If is_always_equal, this is a dead branch and a non-instantiation.
...
public:
    typedef function FLUXTMP_UGLYY(wrapper_type);
    FLUXTMP_MSVC_FIX( typedef typename wrapper::wrapper_base wrapper_base; ) // Help MSVC with name lookup through inheritance and friendship.
	...
	/* **flux-MODIFIED** Can't figure out why the following would be required. 
	 * function& should just bind to one of the two assignment operators above
	 * (which it also uses internally). Seems only to be here to have MSVC barf
	 * up warnings about multiple assignment operators.
	 */
...
public:
    typedef unique_function FLUXTMP_UGLYY(wrapper_type);
    FLUXTMP_MSVC_FIX( typedef typename wrapper::wrapper_base wrapper_base; )
    using wrapper::wrapper;
...
public:
    typedef function_container FLUXTMP_UGLYY(wrapper_type);
    FLUXTMP_MSVC_FIX( typedef typename wrapper::wrapper_base wrapper_base; )
    using FLUXTMP_MSVC_FIX (FLUXTMP_UGLYY(common_allocator) =) typename function_container::wrapper::FLUXTMP_UGLYY(common_allocator);
...
public:
    typedef unique_function_container FLUXTMP_UGLYY(wrapper_type);
    FLUXTMP_MSVC_FIX( typedef typename wrapper::wrapper_base wrapper_base; )
    using FLUXTMP_MSVC_FIX (FLUXTMP_UGLYY(common_allocator) =) typename unique_function_container::wrapper::FLUXTMP_UGLYY(common_allocator);
...
bool operator == ( FLUXTMP_UNPACK NAME const & a, std::nullptr_t ) \
    { return !a; } \
template< __VA_ARGS__ > \
bool operator != ( FLUXTMP_UNPACK NAME const & a, std::nullptr_t ) \
    { return (bool) a; } \
template< __VA_ARGS__ > \
bool operator == ( std::nullptr_t, FLUXTMP_UNPACK NAME const & a ) \
    { return !a; } \
template< __VA_ARGS__ > \
bool operator != ( std::nullptr_t, FLUXTMP_UNPACK NAME const & a ) \
    { return (bool) a; } \
template< __VA_ARGS__ > \
void swap( FLUXTMP_UNPACK NAME & lhs, FLUXTMP_UNPACK NAME & rhs ) \
...
#undef FLUXTMP_deprecated
#undef FLUXTMP_OLD_GCC_FIX
#undef FLUXTMP_OLD_GCC_SKIP
#undef FLUXTMP_MSVC_FIX
#undef FLUXTMP_MSVC_SKIP
#undef FLUXTMP_UNPACK
#undef FLUXTMP_IGNORE
}
